...
<div class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
    ...
    {{-- Judul Halaman --}}
    <h3 class="text-2xl font-bold mb-6 text-gray-800">Tambah Obat Baru</h3>
    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <div class="p-6 sm:px-8 bg-white border-b border-gray-200">
            ...
            {{-- Formulir Tambah Obat. Mengarah ke route 'obat.store' dengan method POST --}}
            <form action="{{ route('obat.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    {{-- Di sini Anda bisa menambahkan Blade directive untuk menampilkan error validasi jika ada --}}
                ...
                <!-- Apotek (Dropdown Select) -->
                {{-- Ini adalah field relasi yang memerlukan koleksi $apoteks dari controller --}}
                <div class="mb-4">
                    ...
                    <select name="apotek_id" id="apotek_id" 
                        ...
                        {{-- Loop untuk menampilkan semua apotek sebagai opsi --}}
                        @foreach ($apoteks as $apotek)
                            <option value="{{ $apotek->id }}" {{ old('apotek_id') == $apotek->id ? 'selected' : '' }}>
                                {{ $apotek->nama_apotek }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center">
                        <input type="text" name="cara_minum" id="cara_minum" 
                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                            value="{{ old('cara_minum') }}" required
                            placeholder="Contoh: 3x">
                        <span class="ml-2 text-gray-700 whitespace-nowrap">sehari</span>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="waktu_minum" id="waktu_minum" 
                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        value="{{ old('waktu_minum') }}" required
